...
import 'package:stylish/core/payment/stripe/customer_session_model.dart';
import 'package:stylish/core/payment/stripe/payment_intent_model/payment_intent_model.dart';
...
class StripeManager extends StripeManagerInterface {
  ...
  Future<PaymentIntentModel> _createPaymentIntent({
    ...
    final response = await _dioConsumer.post(
      ...
      data: {
        ...
        "currency": currency,
        "customer": await getIt.get<CacheHelper>().getSecureData(
          key: CacheKey.stripeCustomerId,
        ),
      },
      headers: headers,
    );
  ...
  Future<CustomerSessionModel> _getEphemeralKey() async {
    String? customerId = await getIt.get<CacheHelper>().getSecureData(
      key: CacheKey.stripeCustomerId,
    );
